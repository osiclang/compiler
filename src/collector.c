...
void
collector_stack_push(struct osic *osic, struct lobject *object)
{
	struct collector *collector;
...
void
osic_collector_mark(struct osic *osic, struct lobject *object)
{
...
void
collector_mark_children(struct osic *osic, struct lobject *object)
{
	GC_SET_MARK(object);
	GC_CLR_GRAY(object);
	lobject_method_call(osic, object, LOBJECT_METHOD_MARK, 0, NULL);
}

void
osic_collector_trace(struct osic *osic, struct lobject *object)
{
	struct collector *collector;
...
void
osic_collector_untrace(struct osic *osic, struct lobject *object)
{
	uintptr_t marked;
	struct collector *collector;
	struct lobject *curr;
                        ...
                        struct lobject *b)
{
	...
	collector = osic->l_collector;
	if (lobject_is_pointer(osic, b) && GC_HAS_MARK(a) && !GC_HAS_MARK(b)) {
		/*
