...
GOOS=windows GOARCH=amd64 go build -o ../bin/windows/occ.exe ../cmd/main.go
fi
if [[ $dist == 'osx' || $dist == 'all' ]]; then
echo -e "Build binary for OSX"
GOOS=darwin GOARCH=amd64 go build -o ../bin/osx-amd/occ ../cmd/main.go
GOOS=darwin GOARCH=arm64 go build -o ../bin/osx-arm/occ ../cmd/main.go
fi
if [[ $dist == 'linux' || $dist == 'all' ]]; then
echo -e "Build binary for linux"
GOOS=linux GOARCH=amd64 go build -o ../bin/linux/occ ../cmd/main.go
fi
if [[ $dist == 'bsd'  || $dist == 'all' ]]; then
echo -e "Build binary for FreeBSD"
GOOS=freebsd GOARCH=amd64 go build -o ../bin/freebsd/occ ../cmd/main.go
fi